...
from net.Attention import attention_resnet
bn_momentum = 0.1
affine = True
...
class FeatureNet(nn.Module):
    def __init__(self, config):
        ...
        self.reduce2 = nn.Sequential(
            ...
            nn.BatchNorm3d(64, momentum=bn_momentum),
            nn.ReLU(inplace=True))
        self.Attngate1 = Attention_gate(F_g=64,F_l=64,F_int=64)
        # upsampling
        ...
        self.reduce3 = nn.Sequential(
            ...
            nn.BatchNorm3d(64, momentum=bn_momentum),
            nn.ReLU(inplace=True))
        self.Attngate2 = Attention_gate(F_g=64,F_l=64,F_int=64)
    ...
    def forward(self, x):
        ...
        rev3 = self.Attngate1(g=out3,x=rev3)
        comb3 = self.back3(torch.cat((rev3, out3), 1))
        rev2 = self.path2(comb3)
        out2 = self.reduce3(out2)
        rev2 = self.Attngate2(g=out2,x=rev2)
...
class CropRoi(nn.Module):
    ...
    def forward(self, f, inputs, proposals):
        ...
        for p in proposals:
            ...
            fe1 = comb2[b, :, int(z_start / 4):int(z_end / 4), int(y_start / 4):int(y_end / 4), int(x_start / 4):int(x_end / 4)].unsqueeze(0)
            fe1_up = self.up2(fe1)

            fe2 = self.back2(torch.cat((fe1_up, out1[b, :, int(z_start / 2):int(z_end / 2), int(y_start / 2):int(y_end / 2), int(x_start / 2):int(x_end / 2)].unsqueeze(0)), 1))
            # fe2_up = self.up3(fe2)
...
class CA-Net(nn.Module):
    def __init__(self, cfg, mode='train'):
        super(CA-Net, self).__init__()
